...
import { Wrench, CalendarIcon, Filter, Sparkles, TrendingDown, Hash, DollarSign } from "lucide-react";
import { cn } from "@/lib/utils";

interface MaintenanceReportData {
  records: (Maintenance & { truck?: Truck })[];
  totals: { count: number; totalValue: number; avgValue: number };
  byType: { type: string; value: number }[];
  byTruck: { truck: string; value: number }[];
}

const COLORS = ["#f97316", "#06b6d4", "#8b5cf6", "#10b981", "#ef4444", "#f59e0b", "#ec4899", "#6366f1"];
...
export default function MaintenanceReportPage() {
  ...
  const { data, isLoading } = useQuery<MaintenanceReportData>({
    queryKey: ["/api/reports/maintenance", queryParams.toString()],
  });
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className="text-3xl font-bold tracking-tight" data-testid="text-report-maintenance-title">
          Relatório de Manutenção
        </h1>
        <p className="text-muted-foreground mt-1">Análise completa dos gastos com manutenção da frota</p>
      </div>
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !startDate && "text-muted-foreground")} data-testid="button-maint-start-date">
            ...
            <div className="space-y-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className={cn("w-full justify-start text-left font-normal", !endDate && "text-muted-foreground")} data-testid="button-maint-end-date">
                    <CalendarIcon className="mr-2 h-4 w-4" />
            ...
            <div className="space-y-2">
              ...
              <Select value={selectedTruck} onValueChange={setSelectedTruck}>
                <SelectTrigger data-testid="select-maint-truck">
            ...
            <CardContent className="p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-orange-700 dark:text-orange-400" data-testid="stat-maint-total">
            ...
            <CardContent className="p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-violet-700 dark:text-violet-400" data-testid="stat-maint-count">
                    {data.totals.count}
                  </p>
                </div>
              </div>
          ...
          <Card className="border-0 shadow-lg bg-gradient-to-br from-cyan-500/5 to-blue-500/5">
            <CardContent className="p-5">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm text-muted-foreground">Média por Serviço</p>
                  <p className="text-2xl font-bold text-cyan-700 dark:text-cyan-400" data-testid="stat-maint-avg">
                    {formatCurrency(data.totals.avgValue)}
                  </p>
                </div>
              </div>
            </CardContent>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="h-80">
              {data?.byType && data.byType.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={data.byType} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={3} dataKey="value" nameKey="type" label={({ type }) => type}>
        ...
        <Card className="border-0 shadow-lg">
          <CardHeader>
            ...
            <CardDescription>Comparativo de custos entre veículos</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-80">
        ...
        <CardContent className="p-0">
          {data?.records && data.records.length > 0 ? (
              ...
              <Table>
                ...
                <TableBody>
                  {data.records.slice(0, 50).map((record) => (
                    <TableRow key={record.id} data-testid={`row-maint-${record.id}`}>
            ...
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground mt-1">Ajuste os filtros ou cadastre manutenções</p>
            </div>
          )}
        </CardContent>
      </Card>
